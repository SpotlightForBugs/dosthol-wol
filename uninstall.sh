...
# dosthol Uninstall Script
# This script removes the dosthol Wake-on-LAN system from Proxmox VE
...
check_required_files() {
    ...
    # If any files are missing, show warning but continue
    if [[ ${#MISSING_FILES[@]} -gt 0 ]]; then
        print_warning "The following files are missing from the source directory:"
        for file in "${MISSING_FILES[@]}"; do
            echo "  - ${file}"
        done
        echo
        print_warning "This may indicate an incomplete installation or corrupted files."
        print_warning "The uninstaller will attempt to remove installed components anyway."
        echo
